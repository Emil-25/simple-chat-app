...
onMounted(async () => {
  ...
  setTimeout(() => {
    if (!user) {
      info.value = 'You should Log in to chat.'
  ...
  await setDoc(
    doc(usersRef, user.providerData[0].uid),
    {
...
const addToChatList = async (email) => {
  ...
  await setDoc(
    doc(chatRoomsRef, user.providerData[0].uid + docs[0].id),
    {
...
<template>
  <q-layout view="hHh lpR fFf" class="bg-green-100">
    ...
    <q-drawer
      ...
      class="bg-green-200"
    >
      <Profile v-if="users" v-for="user of users" :key="user.email" :email="user.email"></Profile>
      <q-skeleton v-if="!noFriends && users.length == 0 && !info" type="circle" height="50px" />
      <q-separator v-if="!noFriends && users.length == 0 && !info" />
      <q-skeleton v-if="!noFriends && users.length == 0 && !info" type="circle" height="50px" />
      <q-separator v-if="!noFriends && users.length == 0 && !info" />
      <q-skeleton v-if="!noFriends && users.length == 0 && !info" type="circle" height="50px" />
      <q-separator v-if="!noFriends && users.length == 0 && !info" />
