  ...
  <div>
    <div class="h-[calc(100vh-145px)] overflow-scroll overflow-x-hidden">
      <div class="q-pa-md row justify-center">
        <div style="width: 100%">
          <q-chat-message
            v-for="msg of messages.value"
            v-if="messages.value"
            :name="msg.from"
            :avatar="
              msg.from == user.providerData[0].displayName
                ? user.providerData[0].photoURL
                : chattingWith.photoURL
            "
            :sent="msg.from == user.providerData[0].displayName ? true : false"
            :stamp="
              msg.time ? new Date(msg.time.seconds * 1000).toLocaleString().split(',')[1] : 'none'
            "
          >
    ...
    <div
      class="fixed-bottom w-full right-0 sm:mx-auto sm:w-[60%] flex flex-row"
      :class="{ '!ml-[40%]': props.drawerOpen }"
    >
      <q-file
        outlined
        v-model="file"
        ...
        label="Attach image..."
        accept=".jpg, image/*"
        @rejected="alert('Only images are allowed!')"
      >
      ...
      >
        ...
        <template v-slot:hint> Smile) </template>
